...
// mise départ
points += parseInt(prompt("La maison vous offre 5€. Achetez des jetons supplémentaires ici. Pour combien en voulez-vous ? Insérez un montant rond, pas de centimes."));
...
do {
    ...
    if (isNaN(mise)===true || mise ===0) {
        break;
    } else if (mise > 0 && mise <= points){
        randomN = numb[Math.round(Math.random()*(numb.length -1))]; //lancer de la bille
        where = parseInt(prompt(`Où désirez-vous jouez ? 
        1. Intérieur
        2. Extérieur ?`)); //pre switch
        if (isNaN(where)===true|| where < 0 || where >5) {
            alert(`Il semblerait que tu aies décidé d'arrêter de jouer. Tu repars donc avec un total de gains de ${points} euros.`)
            break;
        }
        ...
        switch (where) {
            ...
            // début exterior
            case 2:
                exterior = parseInt(prompt(`Où veux-tu miser ? Ecris le chiffre correspondant : 
                1. La couleur
                2. Pair ou Impair
                3. Sur la colonne
                4. Sur la douzaine
        ...
        alert ("Il semblerait que tu aies surestimé ta cagnotte...")
    }
} while (points > 0 && points < 2000 );
//cloture de la boucle totale
// message de fin
if (points >=2000) {
    document.write(`<h1> You broke the bank! On va faire faillite avec toi. Tu repars avec ${points} euros. </h1>`);
} else if (points <= 0) {
    document.write("<h1> Tu as tout perdu... </h1>");

} else {
document.write(`<h1> La partie est finie ? Tu repars avec ${points} euros. </h1>`);
}
